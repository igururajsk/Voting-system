...
from Constituency import show
from functools import partial
#from test_on_fingureprint import fingure
import serial
...
def main():
    ...
    label1=Label(R1,image=photo_image)
...
def Admin():
    def result_time():
        ...
        aa = mysql.connector.connect(host='localhost', port=3306, user="root", password="root", db="votingsystem")
        query3 = aa.cursor()
        
        # Execute the query to fetch the results
        query3.execute("SELECT bjp, congress, jds FROM result")
        rows = query3.fetchall()
        
        # Initialize the variables to ensure they are always assigned a value
        Bjp = 0
        Congress = 0
        Jds = 0
        
        # Assign values to the variables from the fetched data, if available
        if rows:  # If there are rows returned from the query
            for i in rows:
                Bjp = i[0]
                Congress = i[1]
                Jds = i[2]
        ...
        print("Total Number of votes is ", total, 'BJP : ', Bjp, 'Congress : ', Congress, 'JDS : ', Jds)

        # Determine the winner
        if Bjp >= Congress and Bjp >= Jds:
            print('Bjp has won by ', Bjp, 'votes')
        elif Congress >= Bjp and Congress >= Jds:
            print('Congress has won by ', Congress, 'votes')
        elif Jds >= Bjp and Jds >= Congress:
            print('Jds has won by ', Jds, 'votes')
        else:
            print('Draw')
    ...
    def signup():
        R1.destroy()
        def Signup_db():
            global username,Userid,Votercardno,emails,Consti
            ...
            emailid=""
            #fingure_print='no'
            if username == "" or password ==" " or Userid == "" or Votercardno == "" or emails == "" or phoneno == "" or Adreses == "" or Consti=="":
                ...
                if(Votercardno==votecardnum):
                    ...
                    if username == "" or password=="" or Userid == "" or Votercardno == "" or emails == "" or phoneno == "" or Adreses == "" :
                        ...
                        q2=database()
                        query2 = q2.cursor()
                        data=("INSERT INTO facerec(usernames,password,Uniquepass,Userids,Votercardnos,emails,phonenos,Adresess,LoginAuth,FaceAuth,VoteAuth,Constituency)"#,fingure_print)"0
                        "VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)")
                        data1=(username,password,Uniquepass,Userid,Votercardno,emails,phoneno,Adreses,LoginAuth,FaceAuth,VoteAuth,Consti)#,fingure_print)
        ...
        clicked = StringVar()
        clicked.set( "Select" )
        Constis = OptionMenu( R2 ,clicked , *options )
    ...
    res = Button(R1,text = "Result",width=17,height=2,font=('algerian',15,'bold'),justify='center',bg="light blue",relief=SUNKEN,command=result_time)
    res.place( x =530,y=475)

    image3=Image.open('result.jpg')
    image3=image3.resize((250,250))
    photo_image3=ImageTk.PhotoImage(image3)
    label4=tkinter.Label(R1,image=photo_image3)
...
def verification_frame(VoterId_entryyy):
    ...
    if(LoginAuth=='yes' and VoteAuth=='yes'):
        ...
        def verification_db():
            ...
            Uniquepass_ID_entry=Uniquepass.get()
        
        
            if User_ID_entry == "" or Voter_ID_entry == "" or Uniquepass_ID_entry==''  :
                tkinter.messagebox.showinfo("sorry", "Please complete the required field")
            
            else:
                query3.execute('SELECT * FROM facerec WHERE Userids = %s AND Votercardnos = %s AND Uniquepass = %s ', (User_ID_entry,Voter_ID_entry,Uniquepass_ID_entry ))
                result1=query3.fetchall()
                for row1 in result1:
                    ...
                    Uniquepass_Verify=row1[2]
                    #Email_Verify=row1[5]
                
                print(Username_Verify,VoterId_Verify,Email_Verify)
                if(User_ID_entry==UserId_Verify and Voter_ID_entry==VoterId_Verify and Uniquepass_ID_entry == Uniquepass_Verify):
        ...
        Uniquepass= Entry(R4,width=15,font=("bold",17),show="*",highlightthickness=2,bg="WHITE",relief=SUNKEN)
        Uniquepass.place(x=360, y=290)
...
def finger_print(votev,i=0):
    ...
    while(i<3):
        ser.write(b'50')
        tkinter.messagebox.showinfo("info","Place your finger...")
        time.sleep(4)
        finger_id=ser.readline().decode().strip()
        print(finger_id,'@@@@@@@@@@')
        i+=1
        try:
            if int(finger_id)==f_id:
                tkinter.messagebox.showinfo("info","Finger is matched...")
                break
                return
            elif i==3:
                tkinter.messagebox.showinfo("info","Retry again...!")
                return 
            
            else:
                tkinter.messagebox.showinfo("info","Place your finger is not matched..")
        except:
            tkinter.messagebox.showinfo("info","Place your finger Properlly...")
...
def voting_frame(Username_Verify,VoterId_Verify,Email_Verify,U_password_Verify):
    ...
    def vote_here(un,votev,Consti):
        ...
        for row2 in result2:
            ...
            FaceAuth=row2[9]
            Vote_Auth=row2[10]
            #fingure_print=row2[12]
            
        print(FaceAuth)
        if(FaceAuth=='yes' and Vote_Auth=='yes'):# and fingure_print=='yes'):
            tkinter.messagebox.showinfo("Verified","Voted Allredy!!!")
            system()
            
        
        elif(FaceAuth=='yes' and Vote_Auth=='no'):# and fingure_print=='yes'):
            R5.destroy()
            
            #show(un,Consti,,Userid,email_ver)
            show(Consti,votev)
    ...
    btn2 = Button(R5, text="VOTE_HERE", width=20, height=2,fg="black",font=('algerian',15,'bold'),justify='center',bg="light blue",command=partial(otp_frame,un,votev,email_ver))
