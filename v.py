...
def database():
    conn = sqlite3.connect('votingsystem.db')
    print('connect')
    return conn
...
def Admin():
    def table():
        ...
        with conn:
            cursor=conn.cursor()
        cursor.execute('CREATE TABLE IF NOT EXISTS facerec (Username TEXT,Votercardnos TEXT,emails TEXT,Phoneno TEXT)')
        cursor.execute('INSERT INTO facerec (Username,Password ,Email,Phoneno) VALUES(?,?,?,?)',
                  (usernames,passwords,emails,phonenos,))
        cursor=aa.cursor()
        #cursor.execute("SELECT usernames,Votercardnos,emails FROM `facerec")
        ...
        def Signup_db():
            ...
            if username == "" or password ==" " or Userid == "" or Votercardno == "" or emails == "" or phoneno == "" or Adreses == "" :
                ...
                if(Votercardno==votecardnum):
                    ...
                    if username == "" or password=="" or Userid == "" or Votercardno == "" or emails == "" or phoneno == "" or Adreses == "" :
                        tkinter.messagebox.showinfo("sorry","Pease fill the000 required information")
                    else:
                        q2=database()
                        query2 = q2.cursor()
